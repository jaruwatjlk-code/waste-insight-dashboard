...
export function ReplanPage() {
  ...
  const kpi = useMemo(() => {
    ...
    // นับ unique JO ที่มี Value >= 5,000 (ตรงกับ Action List)
    const bigJoSet   = new Set(filteredDetail.filter(r => (r.Value??0) >= 5000).map(r => r.JO).filter(Boolean))
    const bigJobs    = bigJoSet.size
